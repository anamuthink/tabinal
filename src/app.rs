    ...
    pub fn update_ratio(&mut self, path: &[bool], new_ratio: f32) {
        if path.is_empty() {
            if let LayoutNode::Split { ratio, .. } = self {
                *ratio = new_ratio.clamp(SPLIT_RATIO_MIN, SPLIT_RATIO_MAX);
            }
...
#[derive(Clone, Copy)]
enum SpatialDir { Up, Down, Left, Right }
...
pub struct App {
    ...
    // Keybindings loaded from config
    pub key_bindings: Vec<(config::KeyChord, config::Action)>,
    // UI settings loaded from config
    pub ui_settings: config::UiSettings,
    // Timed status message shown in the file tree footer (text, when set)
    pub status_message: Option<(String, Instant)>,
}

impl App {
    pub fn new(rows: u16, cols: u16, key_bindings: Vec<(config::KeyChord, config::Action)>, ui_settings: config::UiSettings) -> Result<Self> {
    ...
    pub fn relayout_panes(&mut self) -> bool {
        ...
        let tab_h = 1u16;
        let main_h = rows.saturating_sub(tab_h);
    ...
    fn match_action(&self, key: &KeyEvent) -> Option<Action> {
        self.key_bindings.iter()
            .find(|(chord, _)| config::key_matches(chord, key))
            .map(|(_, action)| *action)
    }
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                ...
                if let Some(path) = path {
                    ...
                    let boundary = self.ws().file_tree.root_path.clone();
            ...
            KeyCode::Right => {
                self.ws_mut().file_tree.move_into_child();
                Ok(true)
            }
            KeyCode::Left => {
                self.ws_mut().file_tree.move_to_parent();
                Ok(true)
            }
            _ => Ok(true),
        }
    ...
    fn handle_preview_key(&mut self, key: KeyEvent) -> Result<bool> {
        match (key.modifiers, key.code) {
            ...
            // Horizontal scroll — unmodified arrow keys only.
    ...
    fn open_settings(&mut self) -> Result<()> {
        let Some(config_path) = config::config_file_path() else {
            self.status_message = Some(("Cannot locate config directory".to_string(), Instant::now()));
            return Ok(());
        };

        // Create template if the file doesn't exist yet
        ...
        let editor_var = std::env::var("VISUAL")
            ...
            .unwrap_or_else(|_| {
                if cfg!(windows) { "notepad.exe".to_string() } else { "nano".to_string() }
            });

        // Simple whitespace split handles "vim -u NONE" style EDITOR values
        let mut argv: Vec<String> = editor_var.split_whitespace().map(str::to_owned).collect();
        if argv.is_empty() {
            argv.push(if cfg!(windows) { "notepad.exe".to_string() } else { "nano".to_string() });
        }
        ...
        let pane = Pane::new_with_command(
            pane_id,
            pane_rows,
            pane_cols,
            self.event_tx.clone(),
        ...
        let ws_cwd = cwd.unwrap_or_else(|| std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")));
        let initial_cwd = ws_cwd.canonicalize().unwrap_or_else(|_| ws_cwd.clone());
        ...
        let ws = Workspace {
            ...
            custom_name: None,
            file_tree: FileTree::new(ws_cwd.clone()),
            cwd: ws_cwd,
            initial_cwd,
            panes,
            layout: LayoutNode::Leaf { pane_id },
    ...
    /// Move focus to the nearest panel (pane, filetree, or preview) in the given direction.
    fn focus_pane_spatial(&mut self, dir: SpatialDir) {
        #[derive(Clone, Copy)]
        enum NavTarget { Pane(usize), FileTree, Preview }

        let focus = self.ws().focus_target;
        let focused_id = self.ws().focused_pane_id;
        let pane_rects = self.ws().last_pane_rects.clone();
        let tree_rect = self.ws().last_file_tree_rect;
        let preview_rect = self.ws().last_preview_rect;

        let cur = match focus {
            FocusTarget::FileTree => match tree_rect    { Some(r) => r, None => return },
            FocusTarget::Preview  => match preview_rect { Some(r) => r, None => return },
            FocusTarget::Pane     => match pane_rects.iter().find(|(id, _)| *id == focused_id) {
                Some((_, r)) => *r,
                None => return,
            },
        };
        ...
        let mut candidates: Vec<(NavTarget, Rect)> = pane_rects.iter()
            .map(|&(id, r)| (NavTarget::Pane(id), r))
            .collect();
        if let Some(r) = tree_rect    { candidates.push((NavTarget::FileTree, r)); }
        if let Some(r) = preview_rect { candidates.push((NavTarget::Preview,  r)); }

        candidates.retain(|(t, _)| match (*t, focus) {
            (NavTarget::FileTree, FocusTarget::FileTree) => false,
            (NavTarget::Preview,  FocusTarget::Preview)  => false,
            (NavTarget::Pane(id), FocusTarget::Pane)     => id != focused_id,
            _ => true,
        });
        ...
        let best = candidates.iter()
            .filter(|(_, r)| {
                match dir {
                    SpatialDir::Up    => r.y + r.height <= cur.y && overlaps_h(r, &cur),
                    SpatialDir::Down  => r.y >= cur.y + cur.height && overlaps_h(r, &cur),
                    SpatialDir::Left  => r.x + r.width <= cur.x && overlaps_v(r, &cur),
                    SpatialDir::Right => r.x >= cur.x + cur.width && overlaps_v(r, &cur),
                }
            })
            .min_by_key(|(_, r)| {
                let primary: u16 = match dir {
                    SpatialDir::Up    => cur.y.saturating_sub(r.y + r.height),
                    SpatialDir::Down  => r.y.saturating_sub(cur.y + cur.height),
                    SpatialDir::Left  => cur.x.saturating_sub(r.x + r.width),
                    SpatialDir::Right => r.x.saturating_sub(cur.x + cur.width),
                };
                let secondary: u16 = match dir {
                    SpatialDir::Up | SpatialDir::Down => {
                        let cx = r.x + r.width / 2;
                        (cx as i32 - cur_cx as i32).unsigned_abs().min(u16::MAX as u32) as u16
                    }
                    SpatialDir::Left | SpatialDir::Right => {
                        let cy = r.y + r.height / 2;
                        (cy as i32 - cur_cy as i32).unsigned_abs().min(u16::MAX as u32) as u16
                    }
                };
                (primary, secondary)
            });
        ...
        if let Some(&(target, _)) = best {
            match target {
                NavTarget::Pane(id) => {
                    self.ws_mut().focused_pane_id = id;
                    self.ws_mut().focus_target = FocusTarget::Pane;
                }
                NavTarget::FileTree => {
        ...
        while let Ok(event) = self.event_rx.try_recv() {
            ...
            match event {
                AppEvent::PtyEof(pane_id) => {
                    ...
                    for (tab_idx, ws) in self.workspaces.iter_mut().enumerate() {
                        if let Some(pane) = ws.panes.get_mut(&pane_id) {
                            pane.exited = true;
                            if pane.reload_config_on_exit {
                                needs_reload = true;
                    ...
                    if needs_reload {
                        match config::load_config() {
                            Ok(loaded) => {
                                self.key_bindings = loaded.keybindings;
                                self.ui_settings = loaded.ui;
                            }
                            Err(e) => {
                ...
                AppEvent::PtyOutput(pane_id) => {
                    // Reset the coalesce flag so the reader thread can send the next
                    // wakeup event once new output arrives after this drain cycle.
                    for ws in &mut self.workspaces {
                        if let Some(pane) = ws.panes.get_mut(&pane_id) {
                            pane.pending_output.store(false, std::sync::atomic::Ordering::Relaxed);
                            break;
                        }
                    }
                }
...
fn key_event_to_bytes(key: &KeyEvent) -> Option<Vec<u8>> {
    ...
    match key.code {
        ...
        // Alt+Enter → send newline (\n) for multi-line input (e.g. TUI editors)
