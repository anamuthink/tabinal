...
fn main() -> Result<()> {
    // Detect if running inside another tabinal instance
    if std::env::var("TABINAL").is_ok() {
        eprintln!("tabinal: already running inside a tabinal pane (nested instance not allowed).");
        eprintln!("         Open a new tab with Alt+T (or Ctrl+T) or split with Ctrl+Shift+→ / Ctrl+Shift+↓ instead.");
        std::process::exit(1);
    }
    ...
    if let Some(dir) = std::env::args().nth(1) {
        ...
        if path.is_dir() {
            ...
            eprintln!("tabinal: not a directory: {}", dir);
            std::process::exit(1);
        }
    }
    ...
    // Load config before raw mode so errors can be printed to stderr
    let loaded_config = match config::load_config() {
        Ok(c) => c,
        Err(e) => {
            eprintln!("tabinal: config error: {e}");
            std::process::exit(1);
        }
    };
    let key_bindings = loaded_config.keybindings;
    let ui_settings = loaded_config.ui;
    ...
    // Create app
    let mut app = app::App::new(size.height, size.width, key_bindings, ui_settings)?;
